...
def _config():
    ...
    cfg['sources']['bangumi'].update({'enabled':True,'username':'demo','hide_private_collections':True})
...
def _record(source_id: str, private: bool):
    return {'source':'bangumi','source_id':source_id,'category_hint':'anime',
...
def test_private_bangumi_records_are_removed_before_all_outputs(tmp_path: Path):
    ...
    result=run_sync(site,_config(),fixtures)
    ...
    assert 'Item private' not in json.dumps(result,ensure_ascii=False)
