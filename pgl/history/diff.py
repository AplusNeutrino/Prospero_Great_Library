...
def diff_libraries(previous,current,observed_at,timezone_name='UTC'):
    ...
    for cid,new in cur.items():
        ...
        # Steam playtime is cumulative. A newly attached/re-attached source must
        # establish a fresh baseline; otherwise lifetime playtime would be
        # fabricated as a current-year observed delta after an outage/privacy
        # fail-closed interval. A disappearing source is likewise not a
        # negative-playtime correction.
        if ost and nst:
